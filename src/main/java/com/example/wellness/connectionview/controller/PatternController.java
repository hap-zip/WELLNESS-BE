...
public class PatternController {
    ...
    @GetMapping
    public ResponseEntity<List<PatternResponse>> getPatterns(@RequestParam Long userId) {
        return ResponseEntity.ok(patternService.getPatterns(userId));
    }

    @GetMapping("/{patternId}")
    public ResponseEntity<PatternResponse> getPatternDetail(@PathVariable Long patternId) {
        return ResponseEntity.ok(patternService.getPatternDetail(patternId));
    }
}
